...
describe('UserDocumentItem Component', () => {
  ...
  test('renders PDF icon when isPdf is true', () => {
    const pdfProps = { ...defaultProps, isPdf: true, documentUrl: '', documentName: 'pdf-document' };
  ...
  test('fetches document from local chat when documentUrl is not provided', () => {
    ...
    expect(screen.getByAltText('Document thumbnail')).toHaveAttribute('src', 'localChatEntry-test-id');
  });
